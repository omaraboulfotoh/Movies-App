...
interface SerializedPreferencesHolder : PreferencesHolder {
    val gson: Gson
}
...
) : ReadWriteProperty<SerializedPreferencesHolder, T?> {

    private val type = typeToken.type

    @WorkerThread
    override fun getValue(thisRef: SerializedPreferencesHolder, property: KProperty<*>): T? {
        val jsonString = thisRef.preferences.getString(key, null)
        return thisRef.gson.fromJson<T>(jsonString, type)
    }

    override fun setValue(thisRef: SerializedPreferencesHolder, property: KProperty<*>, value: T?) {
        thisRef.preferences.edit {
            putString(key, thisRef.gson.toJson(value, type))
        }
    }
}
...
/**
 * Property delegate for getting and setting an object shared preference.
 *
 * Example usage:
 * ```
 * class Settings : SerializedPreferencesHolder {
 *     ...
 *     val user = User()
...
fun <T> objectPreference(
    ...
    ObjectPreference(key, typeToken)
